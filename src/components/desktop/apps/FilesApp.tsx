...
export function FilesApp({ window: _win }: { window: WindowInstance }) {
  ...
  async function handleCopySelected() {
    const selectedEntries = entries.filter(
      (e) => selectedIds.has(e.id) && e.kind !== "folder" && e.cdnUrl,
    );
    ...
    try {
      await Promise.all(
        selectedEntries.map(async (entry) => {
          setUploadProgress({ label: `Copying ${entry.name}`, pct: 0 });
          // Download original
          const response = await fetch(entry.cdnUrl!);
          if (!response.ok)
            throw new Error(`Failed to download ${entry.name}`);
          const blob = await response.blob();
          const copyName = `copy of ${entry.name}`;
          const file = new File([blob], copyName, {
            type: entry.mimeType || blob.type || "application/octet-stream",
          });
          // Sign + upload
          const { entry: newEntry, uploadUrl } =
            await FileSDKClient.signUpload(
              file.name,
              file.type,
              currentFolderId,
            );
          ...
          await FileSDKClient.confirmUpload(newEntry.id, file.size);
        }),
      );
      setSelectedIds(new Set());
      await reloadCurrent();
