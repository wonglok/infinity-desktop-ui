  ...
  return (
    <>
      ...
      >
        ...
        >
          <StartIcon
            className={isMobile ? "w-6 h-6 opacity-75" : "w-5 h-5 opacity-75"}
          />
          {!isMobile && (
            <span className="text-[12px] hidden sm:inline font-medium">
              Start
            </span>
          )}
        </button>
              ...
              {windows.map((win) => {
                ...
                return (
                  <button
                    ...
                    onClick={() => {
                      if (win.minimized || !active) focusWindow(win.id);
